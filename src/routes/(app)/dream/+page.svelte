<script lang="ts">
  ...
  import SubconsciousInsight from "$lib/components/SubconsciousInsight.svelte";
        ...
        {#if !isVideoPlaying}
          ...
          >
            <SubconsciousInsight
              {analysisResult}
              userPrompt={message}
              videoGenerationPrompt={analysisResult?.video_prompt || ""}
              {isAnalyzing}
              {isReadyToEnter}
              {isAudioFinished}
              {isGenerating}
              onGenerateVideo={handleGenerateVideo}
              onEnterDream={handleEnterDream}
              onReset={handleReset}
            />
          </div>
        {/if}
